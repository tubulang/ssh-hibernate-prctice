...
<div class="main">
    ...
    <%--<s:fielderror />--%>
    <%--<s:fielderror/>--%>
    <s:form method="post" action="changeTicketAction">
        <%--<h2><s:property value="#ticket.name"/>aaa</h2>--%>

        <%--<s:hidden name="air_ticket.uid" />--%>
        <s:hidden name="air_ticket.tid" />
        <%--<s:textfield name="air_ticket.uid" label="姓名"/>--%>
        <%--<s:textfield name="air_ticket.tid" label="姓名"/>--%>
        <s:textfield name="air_ticket.name" label="姓名"/>
        <s:radio name="air_ticket.sex" list="#{'男':'男','女':'女'}" label="性别"/>
        ...
        <s:submit name="addsubmit" value="修改"/>
